...
router.post("/", authMiddleware, async (req: Request, res: Response) => {
  const { userId, name, email, experience } = req.body; // Updated fields
  if (!userId || !name || !email || !experience) {
    return res.status(400).json({ error: "All fields are required" });
  }

  try {
    const newResume = await Resume.create({ userId, name, email, experience });
